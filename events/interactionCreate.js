    ...
    async execute(interaction) {
        try {
            ...
            if (interaction.isCommand()) {
                ...
                const command = interaction.client.commands.find(cmd => cmd.autocompleteName === interaction.commandName);
                if (!command) {
                    console.error(`No autocomplete command matching ${interaction.commandName} was found.`);
                    return interaction.reply({ content: 'An error occurred while executing this autocomplete command.', ephemeral: true });
                }
